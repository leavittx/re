...
{
	char buf[512];
	snprintf(buf, 512, "%s", glGetString(GL_VENDOR));
	string vendor = string(buf);
	snprintf(buf, 512, "%s", glGetString(GL_RENDERER));
	string renderer = string(buf);
	snprintf(buf, 512, "%s", glGetString(GL_VERSION));
...
bool FBOManager::logFBOErrors(const string& str)
{
