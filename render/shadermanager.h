...
struct StockShaderUniforms
{
	...
	StockShaderUniforms(float* _mvpMatrix, float* _vColor) : mvpMatrix(_mvpMatrix), vColor(_vColor) {}
	// Just the texture place (TEXTURE_REPLACE shader)
	StockShaderUniforms(float* _mvpMatrix, int _textureUnit0) : mvpMatrix(_mvpMatrix), textureUnit0(_textureUnit0) {}
	// TEXTURE_MODULATE shader
	StockShaderUniforms(float* _mvpMatrix, float* _vColor, int _textureUnit0) : mvpMatrix(_mvpMatrix), vColor(_vColor), textureUnit0(_textureUnit0) {}
	// DEFAULT_LIGHT shader
	StockShaderUniforms(float* _mvMatrix, float* _pMatrix, float* _vColor) : mvMatrix(_mvMatrix), pMatrix(_pMatrix), vColor(_vColor) {}
	// POINT_LIGHT_DIFF shader
	StockShaderUniforms(float* _mvMatrix, float* _pMatrix, float* _vLightPos, float* _vColor) : mvMatrix(_mvMatrix), pMatrix(_pMatrix), vLightPos(_vLightPos), vColor(_vColor) {}
